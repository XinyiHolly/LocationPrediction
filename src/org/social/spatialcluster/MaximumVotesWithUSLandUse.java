...
public class MaximumVotesWithUSLandUse implements ActivityType {
	...
	public MaximumVotesWithUSLandUse(RelationalDBUtility db, String zoneTable, String tweetsTable, String dbclusterfield, String zonetypefield, int bufferZone, int nexttime) {
		this.db = db;
	...
	public String GetLandusetype(String clusterName, long user_id, int clusterNo) {
		...
		String sql = "select X.zonetype as landusetype, ST_Union(Y.median_geom) as median_geom, ST_Union(ST_Intersection(X.geom, Y.geom)) as geom from " + target + " X, " + zoneTable + " Y "
				   ...
				   + "order by ST_Area(ST_Union(ST_Intersection(X.geom, Y.geom))) desc";
	...
	private String GetZonetype(String landUseType, String clusterName, long user_id, int clusterNo) {
		...
		ArrayList<String> keys = new ArrayList<String>(typecount.keySet());	
		/*if (keys.isEmpty()) {
			return "Others";
		}*/
